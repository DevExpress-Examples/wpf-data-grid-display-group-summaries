...
Namespace DXGrid_DisplayGroupSummaries
	Partial Public Class Window1
		...
		Private Sub CreateGroupSummaries()
			grid.GroupSummary.Add(New GridSummaryItem() With {.FieldName = "Age", _
				.SummaryType = SummaryItemType.Min})
			grid.GroupSummary.Add(New GridSummaryItem() With {.FieldName = "Age", _
				.SummaryType = SummaryItemType.Max})
		End Sub

		Private Sub grid_CustomSummaryExists(ByVal sender As Object, _
				ByVal e As CustomSummaryExistEventArgs)
	...
	Public Class AccountList
		...
		Private Function CreateAccounts() As List(Of Account)
			...
			list.Add(New Account() With {.UserName = "Nick White", _
				.RegistrationDate = DateTime.Today, .Married = True, .Age = 42})
			list.Add(New Account() With {.UserName = "Jack Rodman", _
				.RegistrationDate = New DateTime(2009, 5, 10), .Married = False, .Age = 30})
			list.Add(New Account() With {.UserName = "Sandra Sherry", _
				.RegistrationDate = New DateTime(2008, 12, 22), .Married = False, .Age = 18})
			list.Add(New Account() With {.UserName = "Sabrina Vilk", _
				.RegistrationDate = DateTime.Today, .Married = True, .Age = 24})
			list.Add(New Account() With {.UserName = "Mike Pearson", _
				.RegistrationDate = New DateTime(2008, 10, 18), .Married = True, .Age = 37})
			Return list
		End Function
	End Class
